...
std::string  recivMess(char arryChar[]);
 /* У клиента с сервером есть поля сообения и они будут кодироваться так:
  -- программа имеет немколько состояний:
     ...
     5. Авторизация  на лог и пароль 
     6. Успешная авторизация  лог и пароль  при авторизации
     7. Не успешный лог или пароль при авторизации
     8. Передача сообщения от сервера
     9. Передача сообщения от клиента
  ---Структура сообщения
